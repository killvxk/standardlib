static inline char **append(char ***array, size_t *string_size, const char *element);
...
static inline char *dict_to_string(struct Dict *dict, int pointer_or_string);
...
static inline char *frozen_dict_to_string(struct FrozenDict *dict, int pointer_or_string);
static inline char *frozen_set_to_string(struct FrozenSet *set);
static inline char *int_to_string(int number);
...
static inline char **prepend(char ***array, size_t *count, const char *element);
static inline char *read_bytes_from_file(const char *filename, size_t num_bytes);
static inline char *read_file_into_string(const char *filename, size_t *size);
...
static inline char *set_to_string(struct Set *set);
...
static inline char *string_to_title_case(const char *str);
...
static inline int append_data_to_file(const char *filename, const char *content, size_t content_size);
...
static inline int /* @brief error = -1, no error = 0 */ write_file(const char *filename, const char *content);
static inline int /* @brief success? */ copy_file(const char *source_filename, const char *destination_filename);
static inline int calculate_mode(const int *arr, size_t size);
static inline int calculate_sum(const int *array, size_t size);
static inline int dict_add(struct Dict *dict, const char *key, void *value);
static inline int dict_resize(struct Dict *dict);
static inline int dict_to_array(struct Dict *dict, char ***keys, void ***values, size_t *size_of_keys_and_values);
...
static inline int find_max_length_bitonic_subarray(const int *array, size_t size, size_t *max_length);
...
static inline int frozen_set_to_array(struct FrozenSet *set, char ***keys, size_t *size_of_keys);
static inline int *generate_range(int start, int end, int step, size_t *range_size);
static inline int get_file_size(const char *filename, size_t *size);
...
static inline int remove_file(const char *filename);
static inline int rotate_array(int *array, size_t size, unsigned long positions);
static inline int set_add(struct Set *dict, const char *key);
static inline int set_in(struct Set *dict, const char *key);
static inline int set_resize(struct Set *dict);
static inline int set_to_array(struct Set *set, char ***keys, size_t *size_of_keys);
static inline int **sorted_int_ptrs(int **int_ptrs, size_t size);
static inline int *sorted_ints(int *ints, size_t size);
static inline int string_array_uniq(const char **arr, size_t size, char ***output, size_t *output_size);
...
static inline size_t frozen_dict_to_array(struct FrozenDict *dict, char ***keys, void ***values, size_t *size_of_keys_and_values);
...
static inline void reverse_string_in_place(char *str);
static inline void set_del(struct Set *dict, const char *key);
static inline void set_destructor(struct Set *dict);
static inline void string_array_uniq_adjacent(char *arr[], size_t size);
